...
func (b *DashboardsAPIBuilder) GetGroupVersions() []schema.GroupVersion {
	return []schema.GroupVersion{
		...
		// v2beta2 serves Notebook only, no Dashboard.
		dashv2beta2.NotebookResourceInfo.GroupVersion(),
...
func (b *DashboardsAPIBuilder) InstallSchema(scheme *runtime.Scheme) error {
	...
	if err := dashv2beta2.AddToScheme(scheme); err != nil {
		return err
	}
...
func (b *DashboardsAPIBuilder) Validate(ctx context.Context, a admission.Attributes, o admission.ObjectInterfaces) (err error) {
	...
	// validation.
	case dashv2beta2.NotebookResourceInfo.GroupVersionResource().Resource:
		switch op {
		case admission.Create, admission.Update:
			notebook, ok := a.GetObject().(*dashv2beta2.Notebook)
			if !ok {
...
func (b *DashboardsAPIBuilder) UpdateAPIGroupInfo(apiGroupInfo *genericapiserver.APIGroupInfo, opts builder.APIGroupOptions) error {
	...
	opts.StorageOptsRegister(dashv2beta2.NotebookResourceInfo.GroupResource(), apistore.StorageOptions{
		EnableFolderSupport: true,
	})

	nbStore, err := grafanaregistry.NewRegistryStore(opts.Scheme, dashv2beta2.NotebookResourceInfo, opts.OptsGetter)
	if err != nil {
		return err
	}

	// v2beta2 serves Notebook only, so no other registration creates its storage map.
	notebookStorage := map[string]rest.Storage{
		dashv2beta2.NotebookResourceInfo.StoragePath(): nbStore,
	}
	apiGroupInfo.VersionedResourcesStorageMap[dashv2beta2.VERSION] = notebookStorage
...
func (b *DashboardsAPIBuilder) GetOpenAPIDefinitions() common.GetOpenAPIDefinitions {
	return func(ref common.ReferenceCallback) map[string]common.OpenAPIDefinition {
		...
		maps.Copy(defs, dashv2beta2.GetOpenAPIDefinitions(ref))
	...
	return authorizer.AuthorizerFunc(
		func(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
			if attr.IsResourceRequest() {
				...
				case dashv2beta2.NotebookResourceInfo.GetName():
